...
TEST_PARAM_VERSION = False

if TEST_PARAM_VERSION:
    ...
    pub = rospy.Publisher('/battery_status', BatteryStatus, queue_size=1)

    while not rospy.is_shutdown():
        for i in range(0, 100, 10):
            bs = BatteryStatus()
            bs.percentage = float(i)
            bs.charging = random.choice([True, False])
